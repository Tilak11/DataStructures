 ...
 * BinarySearchTreeMap code given on pages 460-480 in the course textbook
...
//		return tree.toString();
	...
	protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
		...
		public void rotate(Position<Entry<K, V>> p) {
			BinaryTreeNode<Entry<K, V>> x = validate(p);
			BinaryTreeNode<Entry<K, V>> parent = x.getParent();
			BinaryTreeNode<Entry<K, V>> g = parent.getParent();
			if (g == null) {
				setRoot(x);
				x.setParent(null);
			} else {
		 ...
		 * rotated position.
		 * 
		 * @param p the position that represents x in a trinode restructuring of x, its
		...
		public Position<Entry<K, V>> restructure(Position<Entry<K, V>> p) {
			...
			BinaryTreeNode<Entry<K, V>> parent = x.getParent();
			BinaryTreeNode<Entry<K, V>> g = parent.getParent();
			if ((x == parent.getRight()) == (parent == g.getRight())) {
				rotate(parent);
				return parent;
			} else {
				rotate(x);
